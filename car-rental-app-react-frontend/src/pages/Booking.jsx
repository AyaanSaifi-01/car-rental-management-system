...
export default function Booking() {
  ...
  useEffect(() => {
    async function load() {
      const mock = mockCars.find((c) => c.id === Number(id));
      if (mock) { setCar(mock); return; }
      try {
  ...
  const days = Math.max(1, Math.ceil((new Date(form.returnDate) - new Date(form.pickupDate)) / 86400000));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        <form onSubmit={handleSubmit} className="lg:col-span-3 space-y-6">
          <div className="card p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input type="date" name="pickupDate" value={form.pickupDate} onChange={handleChange} className="input-field" required />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1.5">Return Date</label>
                <input type="date" name="returnDate" value={form.returnDate} onChange={handleChange} className="input-field" required />
              </div>
              <div>
          ...
          <button type="submit" disabled={loading} className="btn-primary w-full py-3.5 disabled:opacity-60">
            {loading ? 'Saving to DB...' : 'Confirm Booking → MySQL'}
          </button>
          <p className="text-xs text-center text-gray-400">POST /api/v1/customer/bookCar/{'{id}'} → status PENDING</p>
        </form>
