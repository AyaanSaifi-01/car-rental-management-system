...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(user.role === 'owner' || user.role === 'admin' ? '/admin' : '/');
    } catch (err) {
      setError(err.message || 'Login failed. Backend must be running on port 1571.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-1/2 relative">
        ...
        <div className="relative z-10 flex flex-col justify-end p-12 text-white">
          ...
          <p className="text-lg text-blue-100 max-w-md">Connected to MySQL backend on port 1571.</p>
        </div>
      </div>
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 bg-slate-50">
        <div className="w-full max-w-md">
          <div className="bg-white rounded-2xl shadow-xl p-8 sm:p-10">
            ...
            <p className="mt-4 text-xs text-center text-gray-400 font-mono">API → localhost:1571</p>
          </div>
        </div>
      </div>
    </div>
